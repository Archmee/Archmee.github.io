...
var CategoryListModule = (function() {
    ...
    var getCategoryItemTemplate = function(catItem, startPadding) {
        return  ''
                + '<a id="' + catItem.cid + '" '="" +="" data-id="' + catItem.cid + '" class="item item-cat ' +
                            ((catItem.childCatList.length > 0 || catItem.pid === _topCatId) 
                            ? 'icon-folder' : 'icon-file') + '" style="padding-left:' + startPadding + 'px">'
...
var TodoListModule = (function() {
    ...
    function getTodoListTemplate(todoIds, statuCode) {
        ...
        if (!list || list.length <= 0)="" {="" return="" '';="" }="" list="archiveByDate(list);" 返回后的数据应该排序使用="" var="" keys="Object.keys(list);" keys.sort(function(a,="" b)="" if="" (a=""> b) {
                return -1;
            } else if (a < b) {
                return 1;
            } else {
